...
def align_folder(folder_path, folder_save_path):
    ...
    for img in preds.keys():
        ...
        #print("predpointshape(numface;size):",pred_points.shape)
    ...
    #print("除", avg_points, sumpoints, len(preds))
    ...
    for i, img_pth in enumerate(preds.keys()):
        ...
        #print("wrapped", type(wrapped))
        #cv2.imshow('dst_rt', wrapped)
        #cv2.waitKey(0)
        #cv2.destroyAllWindows()
        #print("foldersavepath:" , folder_save_path)
        img_pth=img_pth.replace("\\","/")

        #print("[0]", img_pth)
        #print("[-1]", img_pth.split('/')[-1])
        #print("[-2]",img_pth.split('/')[-2])
        img_save_path = os.path.join(folder_save_path, img_pth.split('/')[-1])
        #print("imgsavep:", img_save_path)
        img_save_path = img_save_path.replace('\\','/')
        #print("newsavep:",img_save_path)
...
def main():
    ...
    if os.path.isdir(args.folder_path):
        ...
        new_save_img_path = save_img_path.replace("\\",'/')
        #print("folder",args.folder_path)
        #print(new_save_img_path , "saveimgpath:",save_img_path)
        os.makedirs(new_save_img_path, exist_ok=True)

        align_folder(args.folder_path, new_save_img_path)
